...
function makeWordArray(words) {
  ...
  const maxVal = 40; // Set the max value we want in final array
  const minVal = 6; // Set the min value we want in final array
  ...
  console.log(wordCount[0].length);
...
function makeWordCloud(canvas) {
  ...
  const scale = $('#scalefactor').val()/ 100;
  document.getElementById('error').style.display = 'none';
  WordCloud(document.getElementById(canvas), {
    list: cloud,
    rotateRatio: 0, // Odds of word rotation
    fontFamily: 'Raleway', // CSS font
    gridSize: 1, // Spacing between words
    ellipticity: 0.4, // Higher = rounder
    minSize: 7, // Minimum pixel size for a word
    origin: [300, 50], // Starting point in the canvas element
    ...
    weightFactor: (size) => (((size * 40) / 2500) * size + size) * scale,
...
document.getElementById('btn-save').addEventListener('click', function clickSave(e) {
  ...
  elementA.download = 'wordcloud.png';
  elementA.href = url;
  elementA.click();
...
document.getElementById('wordle-canvas').addEventListener('wordclouddrawn', function(event){
  dataService.setDrawnArray(event.detail.drawn);
});

function adjustScaling(log){
  let tempArray = dataService.getDrawnArray();
  for(var i=0; i<tempArray.length; i++){
    if(i<10){
      if(i<10 && tempArray[i]!== true){
        let getCurrentScaling = $('#scalefactor').val();
        let adjustedScaling = getCurrentScaling * 0.90;
        $('#scalefactor').val(adjustedScaling);
        dataService.clearDrawnArray();                  
        makeWordCloud('wordle-canvas');
        break;  
      }
    }else if (i>45){
      if(i>45 && tempArray[i]!==true){
        // alert('CAUTION: Less frequently named words have been cutoff in order to place larger words')
        $('#warning-message').css("visibility", "visible");
        $('#wordle-container').css({'border': '5px solid red'});
        clearInterval(log);
        break;
      }
    }else{      
